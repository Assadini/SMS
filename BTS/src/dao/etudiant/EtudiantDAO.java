...
public class EtudiantDAO {
	...
	public ArrayList<Etudiant> getByclasse(int id) throws SQLException{
		String query="SELECT utilisateur.Nom_Fr,utilisateur.Prenom_Fr,utilisateur.ID,filiere.ID,filiere.code,classe.Label FROM etudiant,utilisateur,filiere,classe,classegenerique WHERE utilisateur.ID=etudiant.UtilisateurID	and etudiant.FiliereID=filiere.ID and classe.ClasseGeneriqueID=classegenerique.ID and classegenerique.FiliereID=filiere.ID and classe.ID=?";
		PreparedStatement statement = (PreparedStatement) con.prepareStatement(query);
		...
		while (rs.next()) {
			...
			etudiant.setPrenom_Fr(rs.getString("utilisateur.Prenom_Fr"));
			
			
			Filiere filiere = new Filiere();
			filiere.setId(rs.getInt("filiere.ID"));
			filiere.setCode(rs.getString("filiere.Code"));
			etudiant.setFiliere(filiere);
