...
public class ClasseDAO {
	...
	public ArrayList<Classe> getAllclassbyprof(int userID) throws SQLException{
		String query="SELECT DISTINCT`professeur`.`UtilisateurID`,   `seancegenerique`.`ProfesseurUtilisateurID`,  `seancegenerique`.`ClasseID`,classe.Label,\r\n"
				+ "`classe`.`ID`, `classe`.`ClasseGeneriqueID`,  `classegenerique`.`ID`, `classegenerique`.`FiliereID` AS filiereID,`classegenerique`.`CycleID` AS cycleID,`cycle`.`Code` AS cycleCode\r\n"
				+ "FROM `professeur` \r\n"
		...
		while (rs.next()) {
			...
			classe.setClasseGenerique(classeGenerique);
			classe.setId(rs.getInt("ClasseID"));
			classe.setLabel(rs.getString("classe.Label"));
			Classes.add(classe);
		}
		rs.close();
		statement.close();
	...
	public ArrayList<Classe> getByfil(int id) throws SQLException{
		String query = "SELECT \r\n" + 
		"	`classe`.`ID` AS `classeID`,\r\n" + 
		"	`classe`.`Label` AS `classeLabel`,\r\n" +
		"	`classegenerique`.`ID` AS `classegeneriqueID`,\r\n" + 
